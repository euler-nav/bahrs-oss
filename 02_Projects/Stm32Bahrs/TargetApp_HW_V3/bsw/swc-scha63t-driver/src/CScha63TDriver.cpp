...
#define MURATA_CSB_UNO_GPIO_Port SPI2_CS0_GPIO_Port
#define MURATA_CSB_UNO_Pin SPI2_CS0_Pin

#define MURATA_CSB_DUE_GPIO_Port SPI2_CS1_GPIO_Port
#define MURATA_CSB_DUE_Pin SPI2_CS1_Pin
...
{
  static CScha63TDriver soInstance{getSpiHandle()};
  ...
  if (bIsInitialized_ && bGotNewDataset)
  {
    ...
    if (iMeasurementCount > 0U)
    {
      ...
      */
      ...
      oImuData.uImuValid_ = BoolToUint(true);
    }
    else
    {
      oImuData = SImuMeasurement();
    }
  }

  orRte.oPortImuInput1_.Write(oImuData);
}
...
uint32_t CScha63TDriver::spiWriteRead(uint32_t uDataOut, GPIO_TypeDef* opGpioX, uint16_t uGpioPin)
{
  ...
  do
  {
    eMurataSpiState = HAL_SPI_GetState(&orSpiHandle_);
  }
  while((eMurataSpiState != HAL_SPI_STATE_READY) && ((GetMicroseconds() - uStartWaitingTime) < 20U));

  if (eMurataSpiState == HAL_SPI_STATE_READY)
  {
    eMurataSpiStatus = HAL_SPI_TransmitReceive(&orSpiHandle_, &uDataBytes[0], &uRespBytes[0], sizeof(uResp), 2);
