...
{
  return writeToAddress(BMP384_REG_INT_CTRL, BMP384_INT_CONF_VALUE);
}
...
{
  return CSpiHandler::GetInstance().Transmit(CSpiHandler::EPeripheral::eBmp384, &uWriteAddr, 1U, &uVal, 1U, 2U);
}

bool CBmp384Driver::readFromAddress(uint8_t* upBuffer, uint8_t uReadAddr, uint16_t uNumByteToRead)
{
  uint8_t uTxData[2]{};
  uTxData[0] = uReadAddr | 0x80; // Set SPI read flag
  uTxData[1] = 0x00; // dummy byte rejection

  return CSpiHandler::GetInstance().Receive(CSpiHandler::EPeripheral::eBmp384, uTxData, 2U, upBuffer, uNumByteToRead, 2U);
}
