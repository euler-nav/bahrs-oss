...
%   dataName  : string label used in figure titles
  ...
  for k = 1:3
    ...
    t_invalid = 1e-6 * p.uTimestampUs_(maskInvalid);
    t_valid = p.uTimestampUs_(maskValid);
    t_valid = 1e-6 * double(t_valid) - 1e-6 * double(t_valid(1));
    col      = colors{k};
    ...
    if any(maskValid)
      plot(t_valid, press(maskValid), validStyle{k}, ...
           'Color', col, 'MarkerSize', 3, 'HandleVisibility', 'off');
    end
    if any(maskInvalid)
      plot(t_invalid, press(maskInvalid), invalidStyle{k}, ...
    ...
    if any(maskValid)
      plot(t_valid, temp(maskValid), validStyle{k}, ...
           'Color', col, 'MarkerSize', 3, 'HandleVisibility', 'off');
    end
    if any(maskInvalid)
      plot(t_invalid, temp(maskInvalid), invalidStyle{k}, ...
           'Color', col, 'MarkerSize', 6, 'LineWidth', 1.5, 'HandleVisibility', 'off');
    end
  end
